...
import picture from "../../assets/pictures/blog3.jpg";
import {AuthContext} from '../../context';
const data=require('../../assets/data.json')

export function Blog3({route, navigation}) {
    ...
    <Image  source={picture} style ={[styles.logo, {height:height*0.3}]} resizeMode="contain"/>
      ...
      <Text style={styles.blog_title}>{data.blogs[2].title}</Text>
      <Text style={styles.content}>{data.blogs[2].content}</Text>
      <Text style={styles.author}>{data.blogs[2].author}</Text>
      <Text style={styles.date}>{data.blogs[2].date}</Text>     
    </View>
    </ScrollView>
    );
}