...
export function viewPdf({route, navigation}) {
  ...
  useEffect(() => {
    fetch('https://project700-backend-neo.herokuapp.com/paper/getPapersInfo')
      .then((response) => response.json())
      .then((responseJson) => {
        papers = []
        responseJson.papers[0].referenced_works?.filter((list, i) => {
            papers.push( {
              "name": list.title,
              "link": list.url
            });
        });
        responseJson.papers[0].related_works?.filter((list, i) => {
          papers.push( {
            "name": list.title,
            "link": list.url
          });
      });
        setReferenceData(papers);
      })
      .catch((error) => {
        console.error(error);
      });
