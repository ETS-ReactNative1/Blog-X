...
import picture from "../../assets/pictures/blog1.png";
import {AuthContext} from '../../context';
const data=require('../../assets/data.json')

export function Blog1({route, navigation}) {
    ...
    <Image  source={picture} style ={[styles.logo, {height:height*0.3}]} resizeMode="contain"/>
      ...
      <Text style={styles.blog_title}>{data.blogs[0].title}</Text>
      <Text style={styles.content}>{data.blogs[0].content}</Text>
      <Text style={styles.author}>{data.blogs[0].author}</Text>
      <Text style={styles.date}>{data.blogs[0].date}</Text>     
    </View>
    </ScrollView>
    );
}